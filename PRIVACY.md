...
_Last updated: 2026-09-19 (P6.4; revised for the P5.10 GIF/sticker panel, vibration strength, and backup opt-out)_
...
- The app holds exactly **two** permissions: `android.permission.INTERNET`, used solely by
  the GIF/sticker panel (GIPHY search/trending and media downloads), and
  `android.permission.VIBRATE`, used for keypress vibration at the strength you choose
  (a "normal" permission — no prompt, no access to any data). No other permission
...
- Both facts are locked by `PrivacyGuaranteeTest` (runs in CI on every push): the manifest
  may hold only INTERNET and VIBRATE, dependencies may not add permissions, and any network
  API used outside the `giphy` package fails the build.
...
- Release checklist: `aapt2 dump permissions app-release.apk` must list exactly
  `android.permission.INTERNET` and `android.permission.VIBRATE`, nothing else.
...
| Settings (theme, key height, feedback toggles, vibration strength) | App-private DataStore | Your preferences | Until you clear app data |

App-private storage is sandboxed by Android: no other app can read these files, and they
are removed when the app is uninstalled. Android auto-backup is switched off
(`android:allowBackup="false"`), so none of these files are ever copied to Google Drive.
